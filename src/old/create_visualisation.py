...
def fetch_graph_data(conn):
    ...
    domain_sizes = {row[0]: row[1] for row in cursor.fetchall()}
    ...
    return relationships, {id: domain for id, domain in domains}, domain_sizes

# Generate graph visualization
def generate_graph(relationships, domain_map, domain_sizes):
    G = nx.DiGraph()

    # Add nodes with sizes based on parent connections
    for domain_id, domain in domain_map.items():
        size = domain_sizes.get(domain, 0)
...
def create_plotly_graph(G, output_html="graph.html", output_png="graph.png"):
    ...
    annotation_bottom_right = dict(
        ...
        x=0.99, y=0.01,  # Adjusted to ensure visibility
        text=watermark_text,
        showarrow=False,
...
def main():
    ...
    try:
        relationships, domain_map, domain_sizes = fetch_graph_data(conn)
        G = generate_graph(relationships, domain_map, domain_sizes)
